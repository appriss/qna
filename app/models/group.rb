class Group
  ...
  include Qna::Models::CustomHtmlMethods
  ...
  referenced_in :qna_version, :class_name => "QnaVersion"
  ...
  def self.find_file_from_params(params, request)
    if request.path =~ /\/(logo|big|medium|small|css|favicon)\/([^\/\.?]+)/
      ...
      logo = @group.has_logo? ? @group.logo : Qna::FileWrapper.new("#{Rails.root}/public/images/logo.png", "image/png")
      case $1
      when "logo"
        logo
      when "big"
  ...
  def version_expired?
    return false if self.qna_version.nil?
